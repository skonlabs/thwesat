...
BEGIN
  ...
  WITH eligible_payments AS (
    ...
    SELECT
      ...
      spr.reviewed_at,
      pa.attributed_at,
      spr.mmk_amount::numeric AS npr,
      public.partner_age_bucket(public.partner_months_between(pa.attributed_at, v_period_end)) AS bucket
    FROM public.subscription_payment_requests spr
    JOIN public.partner_attributions pa ON pa.user_id = spr.user_id
    WHERE pa.partner_id = _partner_id
      AND spr.status = 'approved'
      AND spr.reviewed_at IS NOT NULL
      AND spr.reviewed_at >= v_start
      AND spr.reviewed_at < v_end
      AND spr.reviewed_at >= pa.attributed_at
  )
  SELECT
