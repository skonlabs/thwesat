...
BEGIN
  ...
  -- Force employer_id to caller for safety
  ...
  SELECT
    ...
    COALESCE((SELECT array_agg(value::text) FROM jsonb_array_elements_text(_payload->'categories')), '{}'),
    ...
    COALESCE((SELECT array_agg(value::text) FROM jsonb_array_elements_text(_payload->'payment_methods')), '{}'),
    ...
    COALESCE((SELECT array_agg(value::text) FROM jsonb_array_elements_text(_payload->'skills')), NULL),
  ...
  -- Spend job_post credits (will RAISE if insufficient — rolls back the insert)
...
REVOKE ALL ON FUNCTION public.post_job_with_credits(jsonb, boolean) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.post_job_with_credits(jsonb, boolean) TO authenticated;