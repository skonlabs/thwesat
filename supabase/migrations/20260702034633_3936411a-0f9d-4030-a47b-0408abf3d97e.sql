...
-- Add missing agent_client_id column on jobs (nullable, references JSON client id on agent_profiles.clients)
ALTER TABLE public.jobs ADD COLUMN IF NOT EXISTS agent_client_id uuid;
CREATE INDEX IF NOT EXISTS idx_jobs_agent_client_id ON public.jobs(agent_client_id);

-- Fix post_job_with_quota: validate agent_client_id against JSON array on agent_profiles.clients
  ...
  IF v_client_id IS NOT NULL THEN
    ...
    SELECT EXISTS (
      ...
      FROM public.agent_profiles ap,
           jsonb_array_elements(COALESCE(ap.clients, '[]'::jsonb)) AS c
      WHERE ap.user_id = v_user
        AND (c->>'id')::uuid = v_client_id
    ) INTO v_client_ok;
