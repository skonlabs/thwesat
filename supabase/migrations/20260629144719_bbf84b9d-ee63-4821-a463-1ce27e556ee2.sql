...
BEGIN
  IF TG_OP = 'DELETE' THEN
    DELETE FROM public.wallet_transactions WHERE source_table='topup_requests' AND source_id=OLD.id;
    RETURN OLD;
  END IF;
  ...
  DO UPDATE SET
    credits = EXCLUDED.credits, status = EXCLUDED.status,
    admin_note = EXCLUDED.admin_note, reviewed_by = EXCLUDED.reviewed_by,
    reviewed_at = EXCLUDED.reviewed_at, payment_method = EXCLUDED.payment_method,
    proof_url = EXCLUDED.proof_url, sender_reference = EXCLUDED.sender_reference,
    package_id = EXCLUDED.package_id, mmk_amount = EXCLUDED.mmk_amount,
    updated_at = now();
  RETURN NEW;
END; $$;
