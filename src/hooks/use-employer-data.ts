    ...
    mutationFn: async ({ id, status, rejectionReason, rejectionReasonMy, placementSalary, placementFee, forwardedToEmail, interviewDate }: {
      ...
      if (app) {
        ...
        const notifMap: Record<string, { title: string; titleMy: string; desc: string; descMy: string; type: string }> = {
          ...
          interview: { title: `Interview scheduled`, titleMy: `အင်တာဗျူး ချိန်းဆိုပြီး`, desc: interviewDate ? `Interview for ${jobTitle} on ${new Date(interviewDate).toLocaleString()}` : `An interview has been scheduled for ${jobTitle}`, descMy: interviewDate ? `${jobTitleMy} အတွက် အင်တာဗျူး — ${new Date(interviewDate).toLocaleString()}` : `${jobTitleMy} အတွက် အင်တာဗျူး ချိန်းဆိုပြီး`, type: "application" },
          interviewed: { title: `Interview scheduled`, titleMy: `အင်တာဗျူး ချိန်းဆိုပြီး`, desc: interviewDate ? `Interview for ${jobTitle} on ${new Date(interviewDate).toLocaleString()}` : `An interview has been scheduled for ${jobTitle}`, descMy: interviewDate ? `${jobTitleMy} အတွက် အင်တာဗျူး — ${new Date(interviewDate).toLocaleString()}` : `${jobTitleMy} အတွက် အင်တာဗျူး ချိန်းဆိုပြီး`, type: "application" },
