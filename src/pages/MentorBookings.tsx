...
const MentorBookings = () => {
  ...
  const submitReview = useMutation({
    mutationFn: async () => {
      ...
      if (error) throw error;
      const { data: reviewerProfile } = await (supabase as any).from("v_profiles").select("display_name").eq("id", user.id).maybeSingle();
  ...
  const { data: menteeProfiles = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await (supabase as any).from("v_profiles").select("id, display_name, headline, avatar_url").in("id", menteeIds);
      return data || [];
    },
