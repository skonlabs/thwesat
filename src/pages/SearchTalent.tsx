...
const experienceOptions = [
  ...
  { value: "1-3", labelEn: "1–3 yrs", labelMy: "၁–၃ နှစ်" },
  { value: "4-5", labelEn: "4–5 yrs", labelMy: "၄–၅ နှစ်" },
  { value: "6+", labelEn: "6+ yrs", labelMy: "၆+ နှစ်" },
];
...
const locationOptions = [
  ...
  { value: "Chiang Mai, TH", labelEn: "Chiang Mai", labelMy: "ချင်းမိုင်" },
  { value: "KL, MY", labelEn: "Kuala Lumpur", labelMy: "ကွာလာလမ်ပူ" },
];
...
const SearchTalent = () => {
  ...
  const matchExp = (exp: number) => {
    if (filterExp === "all") return true;
    if (filterExp === "1-3") return exp >= 1 && exp <= 3;
    if (filterExp === "4-5") return exp >= 4 && exp <= 5;
    return exp >= 6;
  };
  ...
  const filtered = mockTalent.filter(t => {
    ...
    const matchesExp = matchExp(t.experience);
    const matchesLoc = filterLocation === "all" || t.location === filterLocation;
    ...
    <div className="min-h-screen bg-background pb-10">
      ...
      <div className="px-5">
        ...
        <AnimatePresence>
          {showFilters && (
            <>
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] bg-black/40" onClick={() => setShowFilters(false)} />
              <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="fixed inset-x-0 bottom-0 z-[60] mx-auto max-w-lg rounded-t-2xl bg-card pb-24">
