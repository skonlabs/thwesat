    ...
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="px-5 space-y-4 mt-4">
        ...
        <div><label className="mb-2 block text-xs font-medium text-foreground">{lang === "my" ? "ငွေပေးချေနည်းများ *" : "Payment Methods *"}</label>
          <div className="flex flex-wrap gap-2">{paymentMethods.map(m => (<button key={m} type="button" onClick={() => togglePayment(m)} className={`rounded-full border px-3 py-1.5 text-xs transition-colors ${selectedPayments.includes(m) ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground"}`}>{m}</button>))}</div>
          <p className="mt-1.5 text-[11px] text-muted-foreground">{lang === "my" ? "လောလောဆယ် Wise ကိုသာ ပံ့ပိုးထားပါသည်" : "Currently only Wise is supported for paying placement fees"}</p>
        </div>
